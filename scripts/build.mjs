...
const plugins = async (plugin) => [
  ...
  {
    name: "lang-define-default-values",
    async transform(code, id) {
      ...
      const lang = Object.entries(langFiles).find(([p]) =>
        id.includes(
          `\\plugins\\${p.replace(/_/g, "-").split(".").slice(0, -1).join(".")}\\`,
        ),
      )?.[1].def;
      ...
      if (lang)
        walk(ast, {
          enter(node) {
            // horrible horrible horrible this will break one day
            if (
              node.type === "CallExpression" &&
              node.callee?.property?.name === "format" &&
              node.callee?.object?.name === "lang"
            ) {
